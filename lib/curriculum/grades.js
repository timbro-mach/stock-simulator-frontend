...
const normalizeModuleBreakdownShape = (moduleSummary) => {
  ...
  const quizScore = numberOrNull(moduleSummary?.quiz ?? moduleSummary?.quizScore ?? moduleSummary?.quiz_score);
  ...
  return {
    ...
    writtenAssignment,
    written_assignment: writtenAssignment,
    writtenTotal: writtenAssignment,
    written_total: writtenAssignment,
    tradeParticipation,
    trade_participation: tradeParticipation,
...
const resolveModuleGradesArray = (payload) => {
  const fromCanonical = getSummaryByModule(payload);
  ...
  return resolveFirstArray(payload, [
    'moduleGradeSummary',
    'module_grade_summary',
    'moduleGrades',
    'module_grades',
...
export const resolveGradeSummaryByModule = (payload) => {
  const modules = resolveModuleGradesArray(payload);
