...
const normalizeModuleBreakdownShape = (moduleSummary) => {
  ...
  const tradeSubmissionSignal = Boolean(tradingObject?.submittedAt ?? tradingObject?.submitted_at);
  const tradeCount = numberOrNull(
    tradingObject?.tradeCount
    ?? tradingObject?.trade_count
    ?? tradingObject?.tradesPlaced
    ?? tradingObject?.trades_placed
    ?? moduleSummary?.tradeCount
    ?? moduleSummary?.trade_count
    ?? moduleSummary?.tradesPlaced
    ?? moduleSummary?.trades_placed,
  );
  // Trades count as "graded" only when there is evidence the user actually placed trades
  // this week. A tradePoints of 0 with no submission signal means no trades were placed.
  const hasTradeActivity = tradeSubmissionSignal
    || (tradeCount !== null && tradeCount > 0)
    || (tradeParticipation !== null && tradeParticipation > 0);
  const tradeStatus = resolveGradingStatus({
    ...
    earnedPoints: hasTradeActivity ? tradeParticipation : null,
    hasSubmissionSignal: tradeSubmissionSignal,
  });
  return {
    ...moduleSummary,
