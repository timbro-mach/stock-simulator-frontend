...
export const getModuleLockState = (module) => {
  const locked = toBool(module?.locked, false);
  const timeUnlocked = toBool(module?.timeUnlocked ?? module?.time_unlocked, true);
  ...
  const unlocksAt = module?.unlocksAt ?? module?.unlocks_at ?? null;
  return {
    locked,
    timeUnlocked,
    prerequisiteMet,
