  ...
  if (assignmentType === 'assignment') {
    return {
      answers: normalizedQuestions.map((question, index) => {
        ...
        // Section-based questions render a single textarea keyed by
        // `questionId`, so submit one response per question. Legacy `parts`
        // content still submits multipart.
        const legacyParts = Array.isArray(question?.parts) ? question.parts : [];
        ...
        const multipartResponses = legacyParts.map((part, partIndex) => {
